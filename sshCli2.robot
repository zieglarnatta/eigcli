...
Execute Ping on 8.8.8.8
    ...
    ...                The keyword returns the standard output by default.
    write               ping 8.8.8.8 -w 4
    Sleep               5
    ${output}=          read until  0% packet loss
    should not be empty  ${output}
    should not contain   ${output}           -ash: ping: not found
...
Execute Traceroute
    ...
    should not contain    ${output}    traceroute: can't set multicast source interface    #has issues
...
Execute ps Processes
    ...
    Sleep                5
    should not be empty     ${output}
    should not contain   ${output}           -ash: ps: not found
...
Execute show interfaces
    ...
    ${output}=         read until prompt
    should not be empty     ${output}
    should not contain   ${output}           -ash: show interfaces: not found
...
Execute show ip route
    ...
    ${output}=         read until prompt
    should not be empty     ${output}
    should not contain   ${output}           -ash: show ip route: not found
...
Execute show iptables
    ...
    ${output}=         read until prompt
    should not be empty     ${output}
    should not contain   ${output}           -ash: show iptables: not found

Execute configure in global and then top to go back to global
    ...
    ${output}=         read until prompt
    should not be empty     ${output}
    should not contain   ${output}           (global)#      -ash: configure: not found
...
Execute config
    [Tags]              Configure       config      config_start
...
Execute ntp server configuration and show it
    [Tags]              Configure       config      ntp     show_ntp
    ...
    should contain  ${output}   (config)#
    #will need to rest the ntps either via

Execute template
    [Tags]              template
    [Documentation]     Execute the __ & return __
    ${execute}=          write        __
    set client configuration  prompt=#
    ${output}=         read until prompt
    should not be empty     ${output}
    should not contain   ${output}           -ash: __: not found
